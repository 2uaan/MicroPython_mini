...
NODE1_MAC_ADDRESS = b'\x88\x57\x21\x69\xf0\x20'
NODE2_MAC_ADDRESS = b'\x5C\x01\x3B\x4B\x41\xCC'
...
while True:
    ...
    if msg:
        ...
        if host == NODE1_MAC_ADDRESS:
            lcd.display_str(f'N1 T:{data_json.get('temp')}C H:{data_json.get('hum')}% ', 0, 0)
            print(f'Received from node 1: {data}')
            print(f'Delay: {delay_time}')
            
        if host == NODE2_MAC_ADDRESS:
            lcd.display_str(f'N2 T:{data_json.get('temp')}C H:{data_json.get('hum')}% ', 0, 1)
